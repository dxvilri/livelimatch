import { XMarkIcon, ArrowLeftOnRectangleIcon, StarIcon as StarIconOutline, MegaphoneIcon, QuestionMarkCircleIcon, SunIcon, MoonIcon, LockClosedIcon } from "@heroicons/react/24/outline";
...
const ProfilePicComponent = ({ profileImage, employerData }) => (
    <div className={`w-12 h-12 rounded-2xl overflow-hidden shadow-lg border select-none shrink-0`}>
        {profileImage ? (
            <img src={profileImage} alt="Profile" className="w-full h-full object-cover" />
        ) : (
            <div className="w-full h-full bg-blue-800 text-white flex items-center justify-center font-black text-lg">
...
export default function Sidebar({ isOpen, setIsOpen, activeTab, setActiveTab, darkMode, setDarkMode, handleLogout, employerData, profileImage, isVerified }) {
  ...
  return (
      <aside className={`fixed top-0 right-0 h-full w-64 z-[100] rounded-l-3xl flex flex-col transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${glassPanel} ${isOpen ? 'translate-x-0 shadow-2xl' : 'translate-x-full lg:translate-x-0'}`}>
        <div className="h-24 flex items-center justify-center relative mt-8 cursor-pointer" onClick={() => { setActiveTab("Profile"); setIsOpen(false); }}>
            <div className={`flex items-center gap-3 p-2 pr-4 rounded-2xl transition-all duration-300 hover:bg-white/10 group`}>
                <ProfilePicComponent profileImage={profileImage} employerData={employerData} />
             ...
             <button onClick={(e) => { e.stopPropagation(); setIsOpen(false); }} className="absolute top-0 right-4 p-2 opacity-50 hover:opacity-100 lg:hidden"><XMarkIcon className="w-6 h-6" /></button>
        </div>

        <nav className="flex-1 px-4 space-y-3 py-4 overflow-y-auto no-scrollbar">
            {isVerified ? (
                <NavBtn active={activeTab==="Ratings"} onClick={()=>{setActiveTab("Ratings"); setIsOpen(false)}} icon={<StarIconOutline className="w-6 h-6"/>} label="Ratings" open={true} darkMode={darkMode} />
            ) : (
                <NavBtn active={false} onClick={()=>{}} icon={<LockClosedIcon className="w-6 h-6 text-slate-500"/>} label="Ratings Locked" open={true} darkMode={darkMode} />
            )}
            <div className={`h-px mx-4 my-2 ${darkMode ? 'bg-white/10' : 'bg-slate-900/10'}`}></div>
            <NavBtn active={activeTab==="Announcements"} onClick={()=>{setActiveTab("Announcements"); setIsOpen(false)}} icon={<MegaphoneIcon className="w-6 h-6"/>} label="Announcements" open={true} darkMode={darkMode} />
            <div className={`h-px mx-4 my-2 ${darkMode ? 'bg-white/10' : 'bg-slate-900/10'}`}></div>
            <NavBtn active={activeTab==="Support"} onClick={()=>{setActiveTab("Support"); setIsOpen(false)}} icon={<QuestionMarkCircleIcon className="w-6 h-6"/>} label="Help & Support" open={true} darkMode={darkMode} />
        </nav>
