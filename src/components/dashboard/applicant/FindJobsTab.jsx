...
export default function FindJobsTab({
    ...
    // Filter logic must happen here based on props if not passed pre-filtered
                                    ...
                                    {JOB_CATEGORIES.map(c => (
                                        <button key={c.id} onClick={() => { setJobCategoryFilter(c.id); setIsCategoryDropdownOpen(false); }} className={`w-full text-left p-3 rounded-lg transition-colors group ${jobCategoryFilter === c.id ? 'bg-blue-600 text-white' : darkMode ? 'hover:bg-white/10' : 'hover:bg-slate-100'}`}>
                                            <div className="flex flex-col">
                                                ...
                                                <span className={`text-[9px] mt-0.5 font-medium truncate ${jobCategoryFilter === c.id ? 'text-white/70' : 'opacity-50'}`}>{c.examples}</span>
                                            </div>
                                        </button>
                                    ))}
                                </div>
                        ...
                        <div key={job.id} onClick={() => onSelectJob(job)} className={`group relative p-6 rounded-[2rem] border transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl cursor-pointer overflow-hidden ${darkMode ? 'bg-slate-900/60 border-white/10 hover:border-blue-500/30' : 'bg-white/80 border-slate-200 hover:border-blue-400/50 hover:shadow-blue-100'} backdrop-blur-xl`}>
                            ...
                            <div className="relative z-10 flex flex-col h-full">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); onToggleSave(job); }} className={`p-2 rounded-full transition-colors ${isSaved ? 'text-blue-500 bg-blue-500/10' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-600'}`}>{isSaved ? <BookmarkIcon className="w-5 h-5 fill-current"/> : <BookmarkIcon className="w-5 h-5"/>}</button>
                                </div>
