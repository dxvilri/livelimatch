...
import { JOB_TYPES, JOB_CATEGORIES } from "../../../utils/applicantConstants";

export default function SavedJobsTab({ 
    savedJobs, myApplications, handleToggleSaveJob, handleApplyToJob, 
    setSelectedJob, darkMode 
}) {
                        ...
                        <div key={item.id} className={`group relative p-6 rounded-[2rem] border transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl cursor-pointer overflow-hidden ${darkMode ? 'bg-slate-900/60 border-white/10 hover:border-blue-500/30' : 'bg-white border-slate-200 hover:border-blue-400/50 hover:shadow-blue-100'} backdrop-blur-xl`}>
                            ...
                            <div className="relative z-10 flex flex-col h-full">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <button onClick={() => handleToggleSaveJob(job)} className={`p-2 rounded-full transition-colors ${hasApplied ? 'opacity-50 cursor-not-allowed text-blue-500 bg-blue-500/10' : 'text-blue-500 bg-blue-500/10'}`} disabled={hasApplied} title={hasApplied ? "Cannot unsave applied jobs" : "Unsave"}>{hasApplied ? <LockClosedIcon className="w-5 h-5"/> : <BookmarkIcon className="w-5 h-5 fill-current"/>}</button>
                                </div>
                                <div className="mb-4">
                                    ...
                                    {job.category && <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wide bg-purple-500/10 text-purple-500 border border-purple-500/20 w-fit"><TagIcon className="w-3 h-3" />{JOB_CATEGORIES.find(c => c.id === job.category)?.label || job.category}</div>}
                                </div>
                                <div className="mt-auto flex gap-4 pt-4 border-t border-dashed border-slate-500/20">
                                    ...
                                    {hasApplied ? (<button disabled className="flex-1 justify-center flex p-3 rounded-xl font-black text-[10px] uppercase tracking-widest bg-green-500/10 text-green-500 cursor-not-allowed">Applied</button>) : (<button onClick={() => handleApplyToJob(job)} className="flex-1 justify-center flex p-3 rounded-xl font-black text-[10px] uppercase tracking-widest bg-blue-600 text-white hover:bg-blue-500 shadow-lg">Apply</button>)}
                                </div>
                            </div>
                        </div>
                    );
