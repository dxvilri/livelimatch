...
export default function EmployerDashboard() {
  ...
  // UPDATED: Removed unreadMsgCount from totalNotifications calculation
...
return (
    <div className={`relative min-h-screen transition-colors duration-500 font-sans pb-24 md:pb-0 select-none cursor-default overflow-x-hidden ${darkMode ? 'bg-slate-950 text-white' : 'bg-slate-100 text-slate-900'}`}>
      ...
      {isJobModalOpen && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 sm:p-6 bg-slate-950/60 backdrop-blur-md animate-in fade-in">
            <div className={`max-w-3xl w-full p-6 sm:p-10 rounded-[3rem] border shadow-2xl overflow-y-auto max-h-[85vh] hide-scrollbar relative ${darkMode ? 'bg-slate-900/90 border-white/10 text-white' : 'bg-white/90 border-white/50 text-slate-900'} backdrop-blur-xl`}>
                ...
                <div className="relative z-10">
                    <div className="text-center mb-10">
                        <h3 className="text-3xl font-black uppercase tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600">{editingJobId ? 'Edit Listing' : 'Create New Job'}</h3>
                        <p className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-50 mt-2">Target your ideal candidate</p>
                    </div>

                    <div className="space-y-8">
                        {/* 1. Job Type Selector */}
                        <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase tracking-widest opacity-60 ml-2">Employment Type</label>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {JOB_TYPES.map((type) => (
                                    <button 
                                        key={type.id} 
                                        onClick={() => setJobForm({...jobForm, type: type.id})} 
                                        className={`
                                            flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all duration-300 relative overflow-hidden group
                                            ${jobForm.type === type.id 
                                                ? 'bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-lg scale-105 ring-2 ring-blue-400 ring-offset-2 ring-offset-transparent' 
                                                : `hover:scale-105 ${darkMode ? 'bg-slate-800/50 hover:bg-slate-800' : 'bg-slate-50 hover:bg-white border border-slate-100 hover:shadow-md'}`
                                            }
                                        `}
                                    >
                                        <span className={`relative z-10 ${jobForm.type === type.id ? 'text-white' : type.color}`}>{type.icon}</span>
                                        <span className="relative z-10 text-[10px] font-black uppercase tracking-widest">{type.id}</span>
                                        {/* Subtle shine for active state */}
                                        {jobForm.type === type.id && <div className="absolute inset-0 bg-white/20 blur-xl"></div>}
                                    </button>
                                ))}
                            </div>
                        </div>

                        {/* 2. Main Details Inputs */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest opacity-60 ml-2">Role Title</label>
                                <input 
                                    type="text" 
                                    placeholder="e.g. Senior Carpenter" 
                                    value={jobForm.title} 
                                    onChange={e => setJobForm({...jobForm, title: e.target.value})} 
                                    className={`w-full p-4 rounded-2xl font-bold text-sm outline-none transition-all focus:ring-2 focus:ring-blue-500/50 ${darkMode ? 'bg-slate-800/50 text-white placeholder-slate-600 hover:bg-slate-800' : 'bg-slate-50 text-slate-900 placeholder-slate-400 hover:bg-slate-100'}`} 
                                />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest opacity-60 ml-2">Monthly Salary</label>
                                <div className="relative group">
                                    <input 
                                        ...
                                        className={`w-full p-4 pl-12 rounded-2xl font-bold text-sm outline-none transition-all focus:ring-2 focus:ring-blue-500/50 ${darkMode ? 'bg-slate-800/50 text-white placeholder-slate-600 hover:bg-slate-800' : 'bg-slate-50 text-slate-900 placeholder-slate-400 hover:bg-slate-100'}`} 
                                    />
                                    <span className={`absolute left-4 top-1/2 -translate-y-1/2 font-black text-xs transition-colors ${darkMode ? 'text-slate-500 group-focus-within:text-blue-400' : 'text-slate-400 group-focus-within:text-blue-500'}`}>PHP</span>
                                </div>
                            </div>
                        </div>
                        
                        {/* 3. Category & Location Dropdowns */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                             {/* Location Dropdown */}
                            <div className="space-y-2 relative">
                                <label className="text-[10px] font-black uppercase tracking-widest opacity-60 ml-2">Target Location</label>
                                <button onClick={() => { setIsLocationDropdownOpen(!isLocationDropdownOpen); setIsJobCategoryDropdownOpen(false); }} className={`w-full p-4 rounded-2xl font-bold text-sm flex justify-between items-center outline-none transition-all focus:ring-2 focus:ring-blue-500/50 ${darkMode ? 'bg-slate-800/50 hover:bg-slate-800 text-white' : 'bg-slate-50 hover:bg-slate-100 text-slate-900'}`}>
                                    <span>{jobForm.sitio || "Select Sitio/Purok..."}</span>
                                    <MapPinIcon className={`w-5 h-5 transition-transform duration-300 ${isLocationDropdownOpen ? 'rotate-180 text-blue-500' : 'text-slate-400'}`} />
                                </button>
                                {isLocationDropdownOpen && (
                                    <div className={`absolute top-full left-0 mt-2 w-full rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 border ${darkMode ? 'bg-slate-900 border-white/10' : 'bg-white border-slate-200'}`}>
                                        <div className="max-h-60 overflow-y-auto hide-scrollbar p-2 space-y-1">
                                            {PUROK_LIST.map(p => (
                                                <button key={p} onClick={() => { setJobForm({...jobForm, sitio: p}); setIsLocationDropdownOpen(false); }} className={`w-full text-left p-3 rounded-xl font-bold text-xs transition-all ${jobForm.sitio === p ? 'bg-blue-600 text-white' : darkMode ? 'hover:bg-white/10 text-slate-300' : 'hover:bg-slate-50 text-slate-700'}`}>
                                                    {p}
                                                </button>
                                            ))}
                                        </div>
                            ...
                            {/* Category Dropdown */}
                            <div className="space-y-2 relative">
                                <label className="text-[10px] font-black uppercase tracking-widest opacity-60 ml-2">Job Category</label>
                                <button onClick={() => { setIsJobCategoryDropdownOpen(!isJobCategoryDropdownOpen); setIsLocationDropdownOpen(false); }} className={`w-full p-4 rounded-2xl font-bold text-sm flex justify-between items-center outline-none transition-all focus:ring-2 focus:ring-purple-500/50 ${darkMode ? 'bg-slate-800/50 hover:bg-slate-800 text-white' : 'bg-slate-50 hover:bg-slate-100 text-slate-900'}`}>
                                    <span>{jobForm.category ? JOB_CATEGORIES.find(c => c.id === jobForm.category)?.label : "Select Category..."}</span>
                                    <TagIcon className={`w-5 h-5 transition-transform duration-300 ${isJobCategoryDropdownOpen ? 'rotate-180 text-purple-500' : 'text-slate-400'}`} />
                                </button>
                                {isJobCategoryDropdownOpen && (
                                    <div className={`absolute top-full left-0 mt-2 w-full rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 border ${darkMode ? 'bg-slate-900 border-white/10' : 'bg-white border-slate-200'}`}>
                                        <div className="max-h-60 overflow-y-auto hide-scrollbar p-2 space-y-1">
                                            {JOB_CATEGORIES.map(c => (
                                                <button key={c.id} onClick={() => { setJobForm({...jobForm, category: c.id}); setIsJobCategoryDropdownOpen(false); }} className={`w-full text-left p-3 rounded-xl transition-all group ${jobForm.category === c.id ? 'bg-blue-600 text-white' : darkMode ? 'hover:bg-white/10 text-slate-300' : 'hover:bg-slate-50 text-slate-700'}`}>
                                                    <div className="flex flex-col">
                                                        <span className="text-xs font-bold">{c.label}</span>
                                                        <span className={`text-[9px] mt-0.5 truncate ${jobForm.category === c.id ? 'text-white/80' : 'opacity-50'}`}>{c.examples}</span>
                                                    </div>
                                                </button>
                                            ))}
                                        </div>
                        ...
                        {/* 4. Description Area */}
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest opacity-60 ml-2">Job Description</label>
                            <textarea 
                                placeholder="Describe the responsibilities, requirements, and benefits..." 
                                value={jobForm.description} 
                                onChange={e => setJobForm({...jobForm, description: e.target.value})} 
                                className={`w-full h-40 p-4 rounded-2xl font-medium text-sm leading-relaxed resize-none outline-none transition-all focus:ring-2 focus:ring-blue-500/50 ${darkMode ? 'bg-slate-800/50 text-white placeholder-slate-600 hover:bg-slate-800' : 'bg-slate-50 text-slate-900 placeholder-slate-400 hover:bg-slate-100'}`} 
                            />
                        </div>
                    </div>

                    {/* Footer Buttons */}
                    <div className="flex gap-4 mt-12 pt-6 border-t border-dashed border-slate-500/20">
                        <button onClick={() => setIsJobModalOpen(false)} className={`flex-1 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all hover:scale-95 ${darkMode ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-100'}`}>Cancel</button>
                        <button onClick={handleSaveJob} className="flex-[2] py-4 rounded-xl font-black uppercase tracking-widest text-[10px] bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 hover:scale-[1.02] active:scale-95 transition-all">
                            {loading ? <div className="flex items-center justify-center gap-2"><div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin"/> Processing...</div> : (editingJobId ? 'Update Job Listing' : 'Publish Job Listing')}
                        </button>
                    </div>
                </div>
            </div>
                    ...
                    {isNotifOpen && isVerified && (
                        <div className={`fixed top-20 left-1/2 -translate-x-1/2 w-[90%] md:absolute md:translate-x-0 md:top-12 md:right-0 md:w-80 md:left-auto rounded-2xl shadow-2xl border overflow-hidden animate-in zoom-in-95 duration-200 z-[100] ${darkMode ? 'bg-slate-800 border-white/10' : 'bg-white border-slate-200'}`}>
                             ...
                             <div className="p-2 space-y-1">
                                 ...
                                 {/* UPDATED: Removed "Unread Messages" button from notification dropdown */}
        ...
        {isVerified && activeTab === "Listings" && (
          <div key="Listings" className="animate-content">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
                <div className={`flex items-center p-2 rounded-2xl border shadow-sm w-full md:max-w-md transition-all focus-within:ring-2 focus-within:ring-blue-500/20 ${glassPanel}`}>
                    <div className="relative flex-1">
                        ...
                        <input 
                            ...
                            className={`w-full bg-transparent border-none outline-none text-sm font-bold placeholder-slate-400 pl-10 pr-4 py-2 ${darkMode ? 'text-white' : 'text-slate-800'}`} 
                        />
                    </div>
                </div>
                                ...
                                <div className="flex -space-x-3 transition-spacing group-hover/applicants:space-x-1">
                                    {[...Array(Math.min(3, applicantCount))].map((_, i) => (
                                        <div key={i} className={`w-9 h-9 rounded-full border-2 flex items-center justify-center text-[10px] font-bold shadow-lg transition-transform hover:scale-110 hover:z-10 ${darkMode ? 'bg-slate-800 border-slate-900 text-white' : 'bg-slate-100 border-white text-slate-600'}`}>
                                            {/* In a real app, map real applicant avatars here */}
                                            ?
                                        </div>
                                    ))}
